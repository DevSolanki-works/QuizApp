...
- **Current Phase:** LIVE on Google Play Store. **iOS build submitted to Apple App Store for review** (August 2026) — TestFlight-tested, Google Sign-In/gameplay/ads all confirmed working on a real device beforehand. Core app + monetization is technically stable after a genuinely rough stabilization stretch (see Decisions Log). **Current bottleneck is growth/distribution, not features or stability.**
...
└── frontend/
├── web/
│   ├── app-ads.txt               ← actively maintained
│   └── privacy.html              ← updated for iOS ATT/IDFA/Unity Ads/Firebase disclosure
└── app/
├── admob.js                  ← RewardedAd, Interstitial (active); RewardedInterstitial (dormant, kept); ATT object (App Tracking Transparency, iOS)
├── analytics.js              ← Firebase Analytics wrapper
├── app-update.js             ← Google Play In-App Update wrapper; confirmed iOS no-op safe by inspection
├── index.html                ← _isSessionLikelyStale(), _markLocalEconomyWrite(), all API.* error paths fixed; safe-area CSS vars + viewport-fit=cover; iOS zoom tuning; ATT boot sequencing; CookieConsent gated web-only
├── supabase-client.js        ← lbUpsertPlayer() fixed, lbDeleteProfile() added
└── screens/
├── results.html           ← soloDeclineDouble() tap-driven ad flow; Custom Topic bonus-generation button removed; Challenge a Friend restyled (.btn-challenge)
├── game.html               ← tutorial 50/50 made server-authoritative via use_powerup + tutorial_demo flag
├── lobby.html              ← tutorial overlay reparented to document.body (WebKit zoom fixed-position fix)
├── home.html               ← highlight tour reparent timing hardened (double-rAF)
├── tutorial-summary.html   ← duplicate forge_show_mode_highlight flag-set removed (Milestone 77)
...
forge/capacitor.config.json  ← ios.contentInset changed to "never"
forge/ios/App/App/Info.plist ← NSUserTrackingUsageDescription added
forge/supabase/migrations/202608080001_leaderboard_read_policy.sql ← public SELECT policy fix
...
| 70 | **Persistent Auth — Supabase Session Migration (LIVE)** — Replaced raw Google ID token auth (which expired every ~1hr with no reliable native silent-refresh path) with Supabase Auth. Native/web sign-in now calls `supabase.auth.signInWithIdToken()`; supabase-js manages long-lived session refresh entirely client-side via `onAuthStateChange()` — no timers, no native plugin calls after initial sign-in. Backend `_verify_google_token()` in `http.py` rewritten to verify Supabase-issued JWTs via `PyJWKClient` against Supabase's JWKS endpoint (ES256 asymmetric signing — see Decisions Log for why this wasn't HS256 as expected), recovering the original Google `sub` from `user_metadata.sub` so every existing `profiles.json`/`leaderboard.google_id`/challenge record kept working unchanged. Confirmed on-device: existing account resumed instantly, brand-new account survived 1hr15min background + Shop + Daily Spin + ticket purchase + gameplay with zero re-auth prompts | **Done — device-tested, confirmed working** |
...
| 77 | Fix Home mode-highlight tour re-triggering `State._inTutorial` after tutorial completion — removed the duplicate `forge_show_mode_highlight` flag-set from `tutorial-summary.html`'s `_tsumMarkCompleted()`, leaving `tutorial-intro.html`'s `_tutProceed()` as the tour's only trigger point | Done — device-tested, confirmed fixed |
| 78 | **Tutorial 50/50 made server-authoritative** — root cause was deeper than the original client-side guess bug: the tutorial's demo power-ups were never real server-side grants at all, so there was no safe way to compute "wrong" options client-side. Rebuilt as a genuine `use_powerup` WebSocket call with a `tutorial_demo: true` flag; server (`websocket.py`) grants it free and one-shot-per-question, gated off once `tutorial_reward_claimed` is true for signed-in accounts, unconditional for guests. Tutorial 50/50 now behaves identically to a real paid use — server decides which 2 wrong options to remove, never the correct one | Done — device-tested, confirmed fixed |
| 79 | **Leaderboard blank-screen bug — root cause confirmed and fixed.** Supabase RLS was missing a public `SELECT` policy on `public.leaderboard` — existing policies allowed row-scoped upserts (a player's own row) but never granted read access across the full table, so the ranked-list query (`ORDER BY ... LIMIT 50`) silently returned empty while a signed-in player's own row/rank continued to update normally. Added `leaderboard_public_read` SELECT policy via migration `202608080001_leaderboard_read_policy.sql`. Also hardened `_lbLoad()` with a hard 8s timeout and real error-detail surfacing so any future failure is diagnosable on-screen instead of presenting as a dead blank list | Done — device-tested, confirmed fixed |
| 80 | **iOS layout overhaul** — `viewport-fit=cover` + `env(safe-area-inset-*)` CSS vars added so screen top bars (back/profile buttons) render below the notch/status bar instead of underneath it; `capacitor.config.json`'s iOS `contentInset` changed to `"never"` (was letting WKWebView auto-inset/shrink the whole page); added a tunable `html[data-forge-os="ios"] .screen { zoom: 1.15 }` to correct WKWebView rendering the fixed-px pixel-font layout smaller than Android's Chrome WebView at identical CSS values — tuned live against a real device across several passes (1.12 → 1.2 → settled at 1.15) | Done — device-tested, confirmed fixed |
| 81 | **iOS tutorial/highlight overlay misalignment (WebKit `zoom` bug) — fixed.** Root cause: WebKit computes `position: fixed` inconsistently for elements nested inside a `zoom`-scaled ancestor — the Lobby tutorial's ring/tooltip/dim overlay are static HTML inside `#screen-lobby` (which now carries the Milestone 80 `zoom`), so their fixed-position coordinates were being computed relative to the zoomed container instead of the real viewport. Fixed by reparenting those three elements to `document.body` on first paint (matching the pattern the Home screen's tour already used, which never had this bug since its overlay elements are created fresh and appended to `document.body` directly) plus a double-`requestAnimationFrame` wait before measuring target coordinates, ensuring the zoomed layout has fully settled first | Done — device-tested, confirmed fixed |
| 82 | **iOS ATT (App Tracking Transparency) flow implemented** — Apple-compliant "soft ask" pre-explainer modal (`#att-explainer-modal`) shown once before the real system tracking prompt, single-path "Continue" CTA (no separate accept/decline that could let someone dodge the real dialog, per Apple 5.1.2(i)). Tapping Continue calls `AdMob.requestTrackingAuthorization()` via a new `ATT` object in `admob.js`; boot sequence (`index.html`) now awaits `_maybeRunATTFlow()` before any AdMob init/preload runs, guaranteeing correct call order. Re-checks silently (no repeat UI) on every subsequent app open, standard ATT behavior. `NSUserTrackingUsageDescription` added to `Info.plist`. Confirmed firing correctly on a real device after ruling out a stale-build/cache false negative on the first attempt | Done — device-tested, confirmed working |
| 83 | **Cookie consent banner gated to web-only** — was showing on every native app first install despite doing nothing functional there (its accept/decline only toggled `window.__adsenseNPA`, which nothing in the native app reads — AdMob, not AdSense, is used in-app). Confusing/alarming for first-time installers and unnecessary since ATT (Milestone 82) is the real, functioning iOS ad-consent mechanism. `CookieConsent.init()` now early-returns unless `PLATFORM.isWebsite` | Done |
| 84 | **Removed Custom Topic "Watch Ad for +1 Free Generation" button** — on iOS, this AdMob rewarded-interstitial ad unit was rendering as an inline button/end-card rather than a full-screen video, producing a 4-button results screen (2X Coins / No Thanks / Watch Ad / Challenge a Friend) that looked cluttered and broken. Removed `customTopicWatchForBonus()` and its button/wrapper entirely from `results.html`; `RewardedInterstitial` in `admob.js` stays dormant/intact per its existing "kept for future revival" status, just with this one active call site removed. Results screen is back to a clean button set | Done |
| 85 | **Challenge a Friend button styling fixed** — was using plain `.btn-secondary` (indistinguishable from a generic Cancel/Back button, read as visually broken/"out of place") with a redundant inline `margin-top` fighting the parent's flex `gap`. Added a dedicated `.btn-challenge` class (yellow-accented border/background/text) and removed the redundant margin | Done |
| 86 | **App Store Connect submission prep** — Age Rating and App Information sections completed. `Info.plist` usage-description audit completed against every native plugin in use (Browser, LocalNotifications, Firebase Messaging, Google Sign-In, AdMob/ATT) — no missing keys found, `NSUserTrackingUsageDescription` was the only real gap and was closed by Milestone 82. Privacy policy (`forgetrivia.online/privacy.html`) updated to cover iOS-specific data flows: ATT/IDFA tracking disclosure, Unity Ads mediation, Firebase Analytics/Cloud Messaging — previously written web/Android-only with an outdated "future Android app" reference. App Store Privacy Nutrition Label drafted (Device ID marked as used for tracking — Yes, matching the ATT implementation; Contact Info/Identifiers/Usage Data disclosed, all other categories Not Collected). Full App Store marketing copy written: subtitle, keywords, promotional text, full description, and App Review Notes (guest-path + Google Sign-In test instructions, AdMob test-ad-unit disclosure, Duel bot-fallback timing note) | Done |
| — | **iOS build submitted to App Store for review** | **Submitted — awaiting Apple review outcome** |
---
...
3. **Per-plugin native iOS config — mostly done, two items still unverified:**
   ...
   - ~~`@capawesome/capacitor-app-update`'s iOS no-op guard~~ — **confirmed by code inspection**: `app-update.js` explicitly checks `getPlatform() === 'ios'` and returns early before any Play Core–specific call, so this is a safe-by-construction no-op rather than something requiring a live exercise.
   - ~~`Info.plist` usage-description strings~~ — **audited, confirmed complete.** Checked against every native plugin in use (Browser, LocalNotifications, Firebase Messaging, Google Sign-In, AdMob). `NSUserTrackingUsageDescription` was the only real gap and was added as part of the ATT implementation (Milestone 82).
...
6. **App Store submission** — **submitted, awaiting Apple review.** Age rating, App Privacy nutrition label, and full marketing copy (subtitle/keywords/promotional text/description/review notes) all completed. Screenshots were the last asset prepared before submission.
...
- **The ~1hr forced re-auth problem — full incident writeup (July 2026):** Google ID tokens expire in ~1hr; the installed native plugin (`@capawesome/capacitor-google-sign-in@0.1.2`) has NO silent-refresh method — confirmed by reading its actual type definitions, not assumed. Three approaches were tried in order:
  1. **Proactive `plugin.signIn()` on a timer/resume hook** — caused a real production incident: `signIn()` can show a genuine interactive account picker, and calling it unprompted in the background created a repeating popup loop that made the app briefly unplayable. Reverted immediately.
  2. **Server-side Google OAuth refresh-token exchange** (`serverAuthCode` → backend exchange → stored refresh token → `/auth/refresh` endpoint) — technically implemented correctly and confirmed working via direct `curl` testing, but **Google only issues a `refresh_token` on a user's very first-ever authorization grant** for a given client+account pair. Since the test account had been through repeated sign-in/sign-out cycles during debugging, Google stopped issuing new refresh tokens on subsequent grants — even forced fresh re-logins didn't get a new one. This made the approach fundamentally untestable/unreliable for any previously-authorized account, which in production is every existing user. Fully abandoned; `google_auth.py` and the `/auth/exchange-code` + `/auth/refresh` endpoints were deleted.
  3. **Supabase Auth (`signInWithIdToken`) — the actual fix, now live.** Supabase issues and silently refreshes its OWN long-lived session, independent of Google's refresh-token quirks entirely. The Google ID token is only ever used once, to bootstrap the Supabase session; native Google Sign-In UI is never touched again afterward for refresh purposes.
...
- **WKWebView `contentInset: "automatic"` was silently shrinking the whole iOS UI:** the default Capacitor iOS setting lets WKWebView auto-inset/scale content to dodge the safe area on its own, which is what made every screen look small on iPhone despite CSS being identical to Android. Fixed by setting `contentInset: "never"` in `capacitor.config.json` (true edge-to-edge rendering) and taking over safe-area handling entirely with `env(safe-area-inset-*)` CSS vars + `viewport-fit=cover`, plus a hand-tuned `zoom: 1.15` to correct WKWebView's genuinely smaller default text/UI rendering versus Chrome WebView at the same px values. **Lesson: a native WebView's own "helpful" auto-adjustment (content insetting, zoom, safe-area compensation) can silently fight hand-written responsive CSS — when a layout looks subtly wrong on one platform only, check the native wrapper's own config defaults before assuming it's a CSS bug.**
- **WebKit `position: fixed` inside a `zoom`-scaled ancestor is unreliable (confirmed twice this session):** first surfaced in the Lobby tutorial overlay after the `zoom` fix above was applied — WebKit computes fixed-position coordinates relative to the zoomed container rather than the true viewport for elements nested inside it. The Home screen's tour never hit this because it builds its overlay elements fresh and appends them directly to `document.body`; the Lobby tutorial's elements were static HTML sitting inside the now-zoomed `#screen-lobby`. Fixed by reparenting to `document.body` on first paint, matching the working pattern, plus a double-`requestAnimationFrame` wait before measuring so the zoomed layout has settled. **Lesson: any UI that measures `getBoundingClientRect()` and renders as `position: fixed` must live outside every zoomed/transformed ancestor, not just "work like the other one that already does" — the two overlays looked structurally similar but only one followed that rule.**
- **ATT false-negative on first test was a stale-build issue, not a code bug:** the pre-explainer modal closed correctly on "Continue" but no native system tracking prompt appeared at all. Root cause was almost certainly a build/cache mismatch — if `NSUserTrackingUsageDescription` isn't present in the actual compiled binary at call time, iOS silently resolves `requestTrackingAuthorization()` as `.denied` with zero UI, no error, no crash. Added a temporary debug toast surfacing the real returned status string, confirmed working correctly after a genuinely fresh delete-and-reinstall from a new Codemagic build off the latest commit; debug toast removed afterward. **Lesson: when a native permission call "does nothing," always rule out stale-binary/cache state via a full uninstall–reinstall before debugging the JS logic — TestFlight/Capacitor builds are especially prone to this since old Info.plist values can persist across an "install over" rather than clean reinstall.**
- **Leaderboard blank-screen bug — root cause confirmed as an incomplete Supabase RLS policy set:** the `leaderboard` table's existing policies (predating the tracked migrations, never fully documented) permitted row-scoped upserts but had no explicit public SELECT policy, so the ranked-list query used by `leaderboard.html` silently returned zero rows while a signed-in player's own row/rank continued to display correctly via the row-scoped `.eq('google_id', ...)` reads in `supabase-client.js`. The asymmetry ("my data updates, the list is blank") was the key diagnostic signal. Fixed via an additive-only migration (`leaderboard_public_read` SELECT policy, `using (true)`) — does not touch or weaken any existing write policy. **Lesson: "my own data works but the aggregate/list view doesn't" is a strong, specific signature of missing SELECT-scope RLS, distinct from a general connectivity or auth failure — worth checking first before assuming a client-side bug.**
- **Tutorial power-ups were never real server grants, which is why the honest fix required a bigger change than expected:** the original tutorial demo power-ups existed only as client-side `State.user.powerups` numbers with no backing profile entry, so there was structurally no way to safely compute "which 2 options are wrong" — the server is the only place `correct_index` is known pre-reveal, by design (anti-cheat). Patching the client-side guess repeatedly could only ever reduce the odds of hiding the correct answer, never eliminate it. Real fix routed the tutorial demo through the actual `use_powerup` WebSocket action with a `tutorial_demo` flag, so the server computes it exactly like a paid use, just free and gated off after onboarding completes. **Lesson: when a demo/preview feature needs to *feel* identical to the real thing, route it through the same authoritative path with a bypass flag rather than re-simulating the real thing's logic client-side — the two will always drift, and here the drift was a correctness bug, not just a cosmetic one.**
